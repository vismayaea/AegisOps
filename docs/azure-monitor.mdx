---
title: "Azure Monitor"
description: "Connect Azure Monitor Log Analytics so OpenSRE can pull KQL log evidence"
---
...
```bash
opensre integrations setup azure
```
...
| `AZURE_LOG_ANALYTICS_ENDPOINT` | `https://api.loganalytics.io` | Override for sovereign clouds (for example `https://api.loganalytics.azure.us` for Azure Government) |
...
Set the resulting token as `AZURE_LOG_ANALYTICS_TOKEN`. Tokens expire (usually after 60 minutes) — see the **Token rotation** note in [Security](#security).
...
| --- | --- | --- |
| `query` | `AppTraces \| order by TimeGenerated desc \| take <limit>` | KQL query text |
| `time_range_minutes` | `60` | Sent as the `timespan` (`PT<N>M`) |
| `limit` | `50` | Per-query row cap; clamped to `max_results` (hard limit `200`) |

OpenSRE always appends a `| take <limit>` clause when one is missing, so the workspace never returns more rows than the configured cap. `AZURE_MAX_RESULTS` defaults to `100` and is hard-capped at `200`.
...
azure     local env   passed   Configured for Azure Log Analytics workspace 00000000-0000-0000-0000-000000000000 via https://api.loganalytics.io.
```
...
| **Wrong endpoint / DNS error** | Sovereign clouds use a different host (for example `https://api.loganalytics.azure.us` for US Government, `https://api.loganalytics.azure.cn` for China). Set `AZURE_LOG_ANALYTICS_ENDPOINT` accordingly. |
