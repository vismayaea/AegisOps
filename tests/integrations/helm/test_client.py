...
from integrations.helm.client import HelmClient, _helm_client_major_version
...
def test_helm_probe_missing_binary(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("integrations.helm.client.shutil.which", lambda _name: None)
    client = _client()
...
def test_helm_probe_passes_when_version_and_list_succeed(
    ...
    monkeypatch.setattr("integrations.helm.client.shutil.which", lambda _name: "/usr/bin/helm")
    ...
    monkeypatch.setattr("integrations.helm.client.subprocess.run", fake_run)
...
def test_helm_list_parses_json_array(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("integrations.helm.client.shutil.which", lambda _name: "/bin/helm")
