"""Direct unit tests for integrations/aws/cloudwatch_client.py service functions.
...
from integrations.aws.cloudwatch_client import (
    filter_log_events,
    get_log_events,
...
def _mock_creds():
    with patch("integrations.aws.cloudwatch_client.require_aws_credentials", return_value=None):
        yield
...
def mock_cw_client():
    client = MagicMock()
    with patch("integrations.aws.cloudwatch_client._get_cloudwatch_client", return_value=client):
        yield client
...
def mock_logs_client():
    client = MagicMock()
    with patch(
        "integrations.aws.cloudwatch_client._get_cloudwatch_logs_client", return_value=client
    ):
        yield client
...
class TestGetMetricStatistics:
    ...
    def test_returns_error_when_no_client(self):
        with patch("integrations.aws.cloudwatch_client._get_cloudwatch_client", return_value=None):
            result = get_metric_statistics(
                namespace="AWS/Batch",
                metric_name="CPUUtilization",
                start_time="2024-01-01T00:00:00Z",
    ...
    def test_returns_error_when_credentials_missing(self):
        with patch(
            "integrations.aws.cloudwatch_client.require_aws_credentials",
...
class TestFilterLogEvents:
    ...
    def test_returns_error_when_no_client(self):
        with patch(
            "integrations.aws.cloudwatch_client._get_cloudwatch_logs_client", return_value=None
        ):
