"""Tests for integrations.aws.s3_client -- direct unit tests with mocked boto3."""
...
from integrations.aws import s3_client
...
def mock_s3() -> Generator[MagicMock]:
    ...
    """
    client = MagicMock()
    p1 = patch("integrations.aws.s3_client.make_boto3_client", return_value=client)
    p2 = patch("integrations.aws.s3_client.require_aws_credentials", return_value=None)
    p1.start()
    p2.start()
    yield client
    p2.stop()
...
class TestBoto3NotAvailable:
    ...
    def _no_boto3(self) -> Generator[None]:
        ...
        p = patch("integrations.aws.s3_client.make_boto3_client", return_value=None)
        p.start()
        yield
        p.stop()
