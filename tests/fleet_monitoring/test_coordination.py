...
def console() -> Console:
    ...
    # visible text (e.g. "/fleet") when CI sets ``FORCE_COLOR`` / TTY-like env.
...
class TestCliCommands:
    """Tests for /fleet claim and /fleet release CLI commands."""
    ...
    def test_release_usage_error(self, session: object, console: Console) -> None:
        ...
        output = console.file.getvalue()
        assert "Usage:" in output
        assert "/fleet release" in output
